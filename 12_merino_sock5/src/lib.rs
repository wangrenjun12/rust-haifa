    ...
    T: AsyncRead + AsyncWrite + Send + Unpin + 'static,
{
    ...
    async fn auth(&mut self) -> Result<(), MerinoError> {
        ...
        if methods.contains(&(AuthMethods::UserPass as u8)) {
            ...
            debug!("Sending NOAUTH packet"); //Step 6
            self.stream.write_all(&response).await?;
            debug!("NOAUTH sent"); //Step 6
            Ok(())
        } else {
    ...
    pub async fn handle_client(&mut self) -> Result<usize, MerinoError> {
        debug!("Starting to relay data");//Step 7
        ...
        info!(
            "New Request: Command: {:?} Addr: {}, Port: {}",  //Step 11
        ...
        match req.command {
            ...
            SockCommand::Connect => {
                debug!("Handling CONNECT Command");//Step 12

                let sock_addr = addr_to_socket(&req.addr_type, &req.addr, req.port)?;

                trace!("Connecting to: {:?}", sock_addr);//Step 13
...
impl SOCKSReq {
        ...
        T: AsyncRead + AsyncWrite + Send + Unpin + 'static,
    {
        ...
        //         order
        trace!("Server waiting for connect");// Step 8
        let mut packet = [0u8; 4];
        // Read a byte from the stream and determine the version being requested
        stream.read_exact(&mut packet).await?;
        trace!("Server received {:?}", packet); //Step 9
